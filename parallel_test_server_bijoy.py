...
def apply_for_TC(window_file,logistic_model,file_name):
    import implement_functions as mf
    ML = window_file.loc[:,'M.level']
    CS = window_file.loc[:,'C.speed']
    CP = window_file.loc[0,'C.percent']
    ...
    temp_x1,temp_x2 = feature_calc(TC_dat[0],ML,CP,CS,logistic_model)
...
if __name__ == '__main__':
    ...
    # importing the model that is already made
...
##        TC_dat = apply_async_TC(file,logistic_model,file_name)
##        end_time = time.time()
...
##        print("total parallel execution time  : ",(end_time-time_start_all))
        ...
        start_time = time.time()
        temp_x1,temp_x2 = apply_for_TC(file,logistic_model,file_name)
        end_time = time.time()
        print("parallel execution time per iteration : ",(end_time-start_time)/(n-63))
        print("total parallel execution time  : ",(end_time-time_start_all))
