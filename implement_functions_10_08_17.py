...
def make_x_300_stay(L1,L2,CS,file_name):
    ...
    if stay >= 0:
        ...
        l1 = L1[iter_index-60:iter_index+stay+1]
        l2 = L2[iter_index-60:iter_index+stay+1]
        cs = CS[iter_index-60:iter_index+stay+1]
        # find_index can be used here as well
        l2_index = 57-stay+1
        l1_index = find_index_l1(l1,l2_index)
        q = make_one_x(l1,l2,cs,l1_index,l2_index,file_name)
        print(max_index,l1_index,l2_index,iter_index)
        print(rise1,rise2,drop1,drop2,drop3,drop4,drop5,drop6)
        print(stay)
    
##        avg_l = np.average(L2[220:230])
##        avg_cs = np.average(CS[220:230])
##        q.extend([avg_l,avg_cs])
##        slope, interce3pt. m,c
##        constants = [[0.011,0.065],[0.039,2.29],[0.357,-5.28],[0.05,5.75],[0.102,13.71],
##                     [0.0577,-4.67],[0.4057,-23.93],[0.039,-1.68],[-0.099,22.90],[-0.042,15.76],[0,0]]
##        q = transform_features(q,avg_l,constants)
        x_temp.append(q)
        y_temp.append(1)
...
##        elif (del1<0)and(del2<0)and(del3==0)and(del4>0)and(del5>0):
##            l2_index -= 1
##            l1_index = find_index_l1(l1,l2_index)
##            tt.append(make_one_prob_level_two(l1,l2,ml,cs,l1_index,l2_index))
##        elif (del1<0)and(del2<0)and(del3==0)and(del4==0)and(del5>0)and(del6>0):
##            l2_index -= 2
##            l1_index = find_index_l1(l1,l2_index)
##            tt.append(make_one_prob_level_two(l1,l2,ml,cs,l1_index,l2_index))
##        elif (del1<0)and(del2<0)and(del3==0)and(del4==0)and(del5==0)and(del6>0)and(del7>0):
##            l2_index -= 3
##            l1_index = find_index_l1(l1,l2_index)
##            tt.append(make_one_prob_level_two(l1,l2,ml,cs,l1_index,l2_index))
##        else:
...
def make_one_prob_level_two(L1,L2,ML,CS,l1_index,l2_index):
    ...
    temp = get_cat(x_temp)
    x_temp.append(temp)
    coef =  [ 0.52266305,  0.07318896, -0.01950882, -0.06894703,  0.18458461,
         0.17205739, -0.0419706 , -0.03886995, -0.22197155,  0.05431946,
        -0.03889197,  0.97026176]
    inter = -2.1806218
