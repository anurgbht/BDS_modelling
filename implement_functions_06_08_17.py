...
def make_one_prob_level_two(L1,L2,ML,CS,l1_index,l2_index):
    ...
    temp = get_cat(x_temp)
    x_temp.append(temp)
    coef = [0.84906148,-0.75157614,0.02240131,-0.01174181,0.24360155,0.82248065,-0.11528301,-0.00856245,-0.25200546,0.20062228,-0.02886105,0.64519583]
    inter = -3.4653837
    ret_tt = inter + np.inner(coef,x_temp)
    ret_tt = np.exp(ret_tt)/(1+np.exp(ret_tt))
    return ret_tt
