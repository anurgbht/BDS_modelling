...
def prelim_logit(*args):
    ...
    if len(args) == 4:
        ...
        thresh = args[2]
        plot_flag = args[3]
        X_train, X_test, y_train, y_test,test_index = train_test_split_my(x_data, y_data, test_size=0.2,random_state=42)
        proceed_flag = 1
    elif len(args) == 6:
        X_train = args[0]
        X_test = args[1]
...
def make_x_3600(L1,L2,ML,CP,CS):
    ...
    for i in range(0,k):
        ...
        cs = CS[i*n:(i+1)*n].reset_index().iloc[:,-1]
        cp = CP[i*n:(i+1)*n].reset_index().iloc[:,-1]
##        del1 = np.abs(l2.iloc[-1] - l2.iloc[-2])
##        del2 = np.abs(l2.iloc[-1] - l2.iloc[-5])
##        del3 = np.abs(l2.iloc[-1] - l2.iloc[-10])
##        if ((del1>2)and(del2>2)and(del3>2)):
        x_temp.append(make_one_x(l1,l2,ml,cp,cs))
##        else:
##            xtemp.append([])
        y_temp.append(0)
