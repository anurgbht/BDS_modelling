...
def rise_slope(l2,tag):
    ...
    for i in range(2,max_index):
        if l2[max_index-i]<l2[max_index-i+2]:
            count += 1
            rise  = (max(l2) -l2[max_index-i])/count
        else:
            break
    return rise
...
def rise_amp(l2,tag):
    ...
    for i in range(2,max_index):
        if l2[max_index-i]<l2[max_index-i+2]:
            rise  = (max(l2) -l2[max_index-i] )
        else:
            break
    return rise

def drop_slope(l2,tag):
    if tag == 1:
        l2 = l2[-25:-5]
    elif tag == 2:
        l2 = l2[-20:]
...
def time_delta(l1,l2,cs):
    l1 = l1[-20:]
    l2 = l2[-20:]
    max_index2 = l2.index(max(l2))
    max_index1 = l1.index(max(l1))
...
def make_x_300(L1,L2,CS,tag):
    ...
    for i in range(k):
        ...
        x_temp.append(make_one_x(l1,l2,cs))
        if i  == 0 and tag == 'True':
            y_temp.append(1)
    return x_temp,y_temp

def make_x_3600(L1,L2,CS,TC_layer):
    L1 = list(L1)
    L2 = list(L2)
    CS = list(CS)
    x_temp = []
    ...
    for i in range(51,len(L1)):
        ...
        if ((rise1>=0)and(rise2>=0)and(rise3>=0)and(drop1>=0)and(drop2>=0)):
            tt = make_one_x(l1,l2,cs)
            tt.extend([i,TC_layer])
            x_temp.append(tt)
            y_temp.append(0)
    return x_temp,y_temp
