...
def my_pred_fun(clf,x_data,y_test,thresh,plot_flag=1):
    ...
    for jj in y_test:
        if jj == 1:
            my_label.append('ones')
        else:
            my_label.append('zeros')
    if plot_flag == 1:
        print('Entering plot function, this one')
        plt.scatter(my_pred,range(0,len(my_pred)),c=y_test,s=120,label=my_label)
        plt.grid(1)
...
def prelim_pca(X,y):
    ...
    X_r1 = X_r.loc[y==1,:]
    X_r0 = X_r.loc[y==0,:]
    plt.scatter(X_r1.iloc[:,0],X_r1.iloc[:,1],c='red')
    plt.scatter(X_r0.iloc[:,0],X_r0.iloc[:,1],c='green')
    plt.show()
    pass

def prelim_lda(X,y):
    print('Preliminary LDA Analysis')
    lda = LinearDiscriminantAnalysis(n_components=2)
    X_r = pd.DataFrame(lda.fit(X).transform(X))
    plt.scatter(X_r[y == 0, 0], X_r[y == 0, 1],c='red')
    plt.scatter(X_r[y == 1, 0], X_r[y == 1, 1],c='green')
    plt.show()
    return X_r
...
def prelim_knn(*args):
    ...
    if len(args) == 5:
        ...
        file_names = file_names.loc[test_index]
        proceed_flag = 1
    elif len(args) == 7:
        X_train = args[0]
        X_test = args[1]
    ...
    if proceed_flag == 1:
        ...
        y_test = [x for x in y_test.iloc[:,0]]
        tt_files = file_names.loc[[((x == 1) and (y == 0)) for x,y in zip(y_test,my_pred)]]
        return clf,tt_files
    else:
...
def prelim_keras(*args):
    ...
    if proceed_flag == 1:
        ...
        clf = Sequential()
        clf.add(Dense(21, input_dim=input_dim, init='uniform', activation='relu'))
        clf.add(Dropout(0.3))
        clf.add(Dense(7, activation='sigmoid'))
        clf.add(Dropout(0.3))
...
def find_index_l1_rise(l1,l2_index):
    l1 = l1[:l2_index]
    tt = find_index(l1)
    if tt < 0:
        tt = l1.index(max(l1))
    qq = tt - rise_time(l1,tt-1)
    return qq
...
def time_delta(l1,l2,cs,max_index1,max_index2):
    if max_index1<max_index2:
        ...
        tt = 0
    return np.abs(tt*(max_index2 - max_index1)-10)
...
def far_amp(l1):
    tt = max(l1[:20])-min(l1[:20])
    return tt

def far_rise(l1,l2_index):
    tt = l1[l2_index] - np.average(l1[:20])
    return tt
...
def prob_level_two(temp_file,active_list,tag):
    ...
    for TC_layer in active_list:
        ...
        del3 = l2[-3] - l2[-4]
        del4 = l2[-4] - l2[-5]
        if (((del1<=0)and(del2>0)and(del3>0)and(del4>0))and(cs[-7]>0)):
            l2_index = len(l2)-2
            l1_index = find_index_l1(l1,l2_index)
...
def make_one_prob_level_two(L1,L2,ML,CS,l1_index,l2_index):
    ...
    # cross = 0.10501421
    coef = [ 0.10891765,  0.52964587,  0.10501421, -0.00787596,  0.49819096,
         0.35376873, -0.00506868, -0.10742236,  0.06623718, -0.40322598]
    inter = -5.1
...
def prob_level_three(temp_file,active_list,i,file_name,tag):
    tt = []
    ml = list(temp_file.loc[:,'M.level'])
    cs = list(temp_file.loc[:,'C.speed'])
    for TC_layer in active_list:
        ...
        del3 = l2[-3] - l2[-4]
        del4 = l2[-4] - l2[-5]
        if (((del1<=0)and(del2>0)and(del3>0)and(del4>0))and(cs[-7]>0.5)):
            l2_index = len(l2)-2
            l1_index = find_index_l1(l1,l2_index)
...
def make_one_prob_level_three(L1,L2,ML,CS,l1_index,l2_index):
    ...
    coef = [ 0.10891765,  0.52964587,  0.30501421, -0.00787596,  0.49819096,
         0.35376873, -0.00506868, -0.10742236,  0.06623718, -0.40322598]
    inter = -5.1
    ret_tt = inter + np.inner(coef,x_temp)
    ret_tt = np.exp(ret_tt)/(1+np.exp(ret_tt))
    ...
    third_level_features = [
            ...
            inst_slope(L1,l2_index),
            inst_slope(ML,l2_index),
            sudden_drop(L1,l2_index),
            far_std(L1),
            far_amp(L1),
            far_std(L2),
            far_amp(L2),
            far_rise(L1,l2_index),
            far_rise(L2,l2_index),
            far_rise(L1,l1_index)
        ]
...
################################################################################
######################## ANN FEATURE GENERATION ###############################
################################################################################

def make_feature_ANN(temp_file,active_list,i,file_name,tag):
    tt = []
    ml = list(temp_file.loc[:,'M.level'])
    cs = list(temp_file.loc[:,'C.speed'])
    for TC_layer in active_list:
        ...
        del3 = l2[-3] - l2[-4]
        del4 = l2[-4] - l2[-5]
        if (((del1<=0)and(del2>0)and(del3>0)and(del4>0))and(cs[-7]>0.5)and(len(l2)==60)):
            plt.subplot(211)
            plt.plot(l2)
            plt.subplot(212)
            plt.plot(cs)
...
def make_one_feature_ANN(L1,L2,ML,CS,l1_index,l2_index):
...
################################################################################
###################### THIRD LEVEL FEATURE FUNCTIONS ############################
################################################################################

def inst_slope(l1,l2_index):
    tt = l1[l2_index] - l1[l2_index-3]
    return tt

def sudden_drop(l1,l2_index):
    tt = l1[l2_index] - max(l1[:l2_index-3])
    return tt
